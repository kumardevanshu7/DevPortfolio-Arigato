...
function renderPortfolio(data) {
    ...
    expListEl.innerHTML = `
      <div class="exp-snake" id="exp-snake">
        <div class="exp-snake-track" aria-hidden="true">
          <svg viewBox="0 0 40 320" preserveAspectRatio="none" class="exp-snake-svg">
            <path d="M20 10 C36 60, 4 120, 20 180 S36 240, 20 300" fill="none" stroke="#E8DCCF" stroke-width="4" stroke-linecap="round"/>
            <path class="exp-snake-fill" d="M20 10 C36 60, 4 120, 20 180 S36 240, 20 300" fill="none" stroke="#C5A484" stroke-width="4.5" stroke-linecap="round"/>
          </svg>
        </div>
        <div class="exp-snake-list">
          ${jobs.map((exp) => {
            const hasLive = Boolean(exp.isLive && exp.siteUrl);
            const liveButton = hasLive
              ? `<a href="${exp.siteUrl}" target="_blank" rel="noopener noreferrer" class="exp-live-pill" title="Visit ${exp.company} live website">
            ...
            const badgeTag = exp.badge
              ? `<div class="exp-badge-wrap"><span class="exp-badge-pill">${exp.badge}</span></div>`
              : '';

            const bulletsHtml = Array.isArray(exp.bullets) && exp.bullets.length
              ? `<ul class="exp-bullets-list">${exp.bullets.map((b) => `<li>${b}</li>`).join('')}</ul>`
              : `<p class="exp-desc">${exp.description}</p>`;

            return `
              <article class="exp-rung" data-exp-id="${exp.id}">
                <button type="button" class="name-pop-btn exp-point ${hasLive ? 'has-live-ring' : ''}" aria-expanded="false" aria-label="${exp.company}">
                  <span class="exp-point-logo ${hasLive ? 'live-logo-glow' : ''}">${exp.mark || exp.company.slice(0, 2)}</span>
                  <span class="name-chip">${exp.company}</span>
                </button>
                <div class="exp-rung-card ${hasLive ? 'has-live-border' : ''}">
  ...
  if (socialListEl) {
    socialListEl.innerHTML = data.socials.map((s) => {
      ...
      if (iconName === 'github') {
      ...
      } else if (iconName === 'globe' || iconName === 'website') {
        iconMarkup = `<svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
        ...
        iconMarkup = `<svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2.2" fill="none" stroke-linecap="round" stroke-linejoin="round">
          <rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect>
          <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path>
          <line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line>
        </svg>`;
      } else if (iconName === 'superprofile') {
        iconMarkup = `<span style="font-family: var(--font-heading); font-weight:900; font-size:14px;">S</span>`;
      } else if (iconName === 'facebook' || iconName === 'meta') {
        iconMarkup = `<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
  ...
  if (toolsBadgeEl) {
    toolsBadgeEl.innerHTML = data.tools.map((t) => {
      ...
      let iconInner = '';
      const iconKey = (t.icon || '').toLowerCase();

      if (iconKey === 'wordpress') {
        iconInner = `<svg viewBox="0 0 24 24" width="22" height="22" fill="#21759B"><path d="M12 0C5.373 0 0 5.373 0 12c0 6.627 5.373 12 12 12s12-5.373 12-12c0-6.627-5.373-12-12-12zm-8.89 12c0-1.74.47-3.37 1.28-4.78L8.6 18.66c-3.23-1.39-5.49-4.76-5.49-8.66zm8.89 8.89c-1.35 0-2.62-.33-3.74-.91l4.08-11.85 4.19 11.48c-.04.03-.08.06-.13.08-1.33.76-2.84 1.2-4.4 1.2zm6.27-5.06l2.79-8.08c.57 1.28.89 2.7.89 4.25 0 3.75-2.09 7.02-5.16 8.57l1.48-4.74zM16.92 6.5c.78 0 1.52.09 1.52.09.61.07.69.97.07.97 0 0-.61-.07-1.29-.07l4.08 12.13C22.42 17.65 23.1 14.93 23.1 12c0-4.99-3.25-9.22-7.8-10.68-.04.45-.16.9-.38 1.34l-2.02 5.84h4.02z"/></svg>`;
      } else if (iconKey === 'instagram') {
        iconInner = `<svg viewBox="0 0 24 24" width="22" height="22"><radialGradient id="ig-tool-grad" r="150%" cx="30%" cy="107%"><stop stop-color="#fdf497" offset="0%"/><stop stop-color="#fdf497" offset="5%"/><stop stop-color="#fd5949" offset="45%"/><stop stop-color="#d6249f" offset="60%"/><stop stop-color="#285AEB" offset="90%"/></radialGradient><rect width="24" height="24" rx="6" fill="url(#ig-tool-grad)"/><circle cx="12" cy="12" r="3.2" fill="none" stroke="#fff" stroke-width="1.6"/><rect x="4.5" y="4.5" width="15" height="15" rx="4" fill="none" stroke="#fff" stroke-width="1.6"/><circle cx="16.5" cy="7.5" r="1" fill="#fff"/></svg>`;
      } else if (iconKey === 'superprofile') {
        iconInner = `<svg viewBox="0 0 24 24" width="22" height="22" fill="none"><rect width="24" height="24" rx="6" fill="#6366F1"/><path d="M7 8.5C7 7.67 7.67 7 8.5 7h7c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5h-5.5v2.5h4c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5h-4v2.5h5.5c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5h-7C7.67 21 7 20.33 7 19.5v-11z" fill="#FFFFFF"/></svg>`;
      } else if (iconKey === 'facebook' || iconKey === 'meta') {
        iconInner = `<svg viewBox="0 0 24 24" width="22" height="22"><rect width="24" height="24" rx="6" fill="#0866FF"/><path d="M16.5 12.2h-2.7v7.8h-3.2v-7.8H8.8v-2.7h1.8V7.8c0-2.4 1.4-3.8 3.6-3.8 1.1 0 2.2.2 2.2.2v2.4h-1.2c-1.2 0-1.6.7-1.6 1.5v1.4h2.8l-.5 2.7z" fill="#FFFFFF"/></svg>`;
      } else if (iconKey === 'firebase') {
        iconInner = `<svg viewBox="0 0 24 24" width="22" height="22"><path fill="#FFA611" d="M3.89 15.67L6.72 2.05a.73.73 0 0 1 1.37-.14l3.19 6.01-7.39 7.75z"/><path fill="#F57C00" d="M14.61 7.23L12.06.67a.73.73 0 0 0-1.35-.04l-2.43 4.63 6.33 1.97z"/><path fill="#FFCA28" d="M20.11 15.67l-2.22-13.62a.73.73 0 0 0-1.28-.35L3.89 15.67l7.35 4.14a1.73 1.73 0 0 0 1.52 0l7.35-4.14z"/></svg>`;
      } else {
        const src = `https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/${t.icon}`;
        iconInner = `<img src="${src}" alt="" width="22" height="22">`;
      }
